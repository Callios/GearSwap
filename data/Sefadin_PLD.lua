...
-- Gear Modes
function user_setup()
    state.OffenseMode:options('Normal', 'Cleave','LowAcc', 'MidAcc', 'HighAcc')
    state.HybridMode:options('Normal', 'HIGH', 'MID', 'LOW')
    state.WeaponskillMode:options('Normal', 'Acc')
    state.CastingMode:options('Normal', 'SIRD', 'HPBAL')    
    state.IdleMode:options('DT', 'Normal', 'Pulling','Cleaving')
...
-- Define three tiers of Defense Taken.  These sets are cycled with the F10 Button.
    ...
    sets.engaged.Cleave = set_combine(sets.idle.Pulling,{main="Malevolence",sub="Priwen",legs="Sakpata's Cuisses",feet="Sakpata's Leggings"})
-- Shield Base
